        ...
        >
            <div className="relative p-5">
                <div className="flex justify-between items-start mb-4">
                    <h3 className="text-lg font-semibold bg-gradient-to-r from-[#FF005D] to-[#00D1FF] bg-clip-text text-transparent
                ...
                <p className="text-[#8a86a0] text-sm mb-5 line-clamp-2">{thread.description}</p>

                <div className="flex items-center justify-between mt-2">
                    <div className="flex items-center space-x-2">
                        <Sparkles size={16} className="text-[#FF005D]" />
                        <span className="text-sm font-medium text-white">{thread.beads.length} Beads</span>
                    </div>
            ...
            <div className="absolute -bottom-2 left-0 w-full h-1 bg-gradient-to-r from-[#FF005D] via-[#00D1FF] to-[#00FFE7]
                      opacity-70 group-hover:opacity-100 transition-opacity pointer-events-none"></div>
       
       <ViewQrCodeModal isOpen={openQrCode} onClose={() => setOpenQrCode(false)} qrURL={thread.qrCode} title={thread.name}/>
        </div>
    );
};
