...
export default function PostHandler() {
    ...
    const pageSize = 2;
    const { data, isLoading, isFetching } = useGetBeadsQuery({ page_number: page, page_size: pageSize });
    const [posts, setPosts] = useState<Post[]>([]);
    const { ref, inView } = useInView();
    const commentBoxRef = useRef<any>(null);
    const [newComment, setNewComment] = useState("");
        ...
        <div className="w-full flex flex-col space-y-8">
            {posts.map((post) => (
                <div key={post.id} className="pt-2 bg-white/5 space-y-2 rounded-md overflow-hidden shadow-sm">
                    ...
                    <div className="bg-white/20 py-3 flex justify-around items-center font-sans text-sm text-white">
                        <button onClick={() => toggleLike(post.id)} className="flex flex-col gap-1 justify-center items-center transition-transform hover:scale-110">
                            {post.isLiked
                                ? <Heart fill="red" color="red" size={22} className="cursor-pointer" />
                                : <Heart size={22} className="cursor-pointer" />}
                            <span>Like</span>
                        ...
                        <button onClick={() => toggleComments(post.id)} className="flex flex-col gap-1 justify-center items-center transition-transform hover:scale-110">
                            <MessageSquareText size={22} className="scale-x-[-1] cursor-pointer" />
                            <span>Comment</span>
                        </button>
